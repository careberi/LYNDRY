  ...
  <header class="site-header">
    <div class="container site-header-bar ops-bar">
      ...
      <nav class="site-nav">
        ...
        ${roles.can(user, 'team.manage') ? tab('/ops/test', 'Test') : ''}
      </nav>
...
// ---------------------------------------------------------------------------
// GET /ops/test — drive the whole system without a working carrier
// ---------------------------------------------------------------------------
//
// Texting does not work until carrier registration is approved, which makes
// the text interface impossible to try and the customer pages impossible to
// reach. This page is the way round that, and it is deliberately the SAME code
// path a real text takes — the AI, the seven actions, the order state machine
// — not a mock of it.
//
// Admin only. It can read any customer's conversation and mint a sign-in code
// for them, so every use is logged with the name of the person who did it.

router.get('/ops/test', guard, may('team.manage'), async (req, res, next) => {
  try {
    ...
    const phone = String(req.query.phone || (customers && customers[0] && customers[0].phone) || '');
    ...
    const reply = req.query.reply ? String(req.query.reply) : '';
    const code = req.query.code ? String(req.query.code) : '';

    const body = `
      ${sectionHeading('Testing', 'Test console')}

      <p style="font-size:17px;line-height:1.6;color:var(--ink-700);max-width:60ch;margin:0 0 32px;">
        Send a message as if it arrived by text. It runs the real thing — the
        same dedupe, the same AI, the same seven actions, the same order rules
        — and shows you what LYNDRY would have replied. Nothing goes near a
        carrier, so this works today.
      </p>

      ${
        reply
          ? `<div class="card card-xl" style="padding:24px;margin-bottom:26px;background:var(--suds-100);">
               <p class="eyebrow" style="margin-bottom:8px;">LYNDRY replied</p>
               <p style="font-size:17px;line-height:1.5;margin:0;">${escapeHtml(reply)}</p>
             </div>`
          : ''
      }
      ${
        code
          ? `<div class="card card-xl" style="padding:24px;margin-bottom:26px;background:var(--sunbeam-500);">
               <p class="eyebrow" style="margin-bottom:8px;color:var(--ink-700);">Customer sign-in code</p>
               <p style="font-family:var(--font-display);font-weight:900;font-size:38px;letter-spacing:0.1em;margin:0 0 8px;">${escapeHtml(code)}</p>
               <p style="font-size:15px;margin:0;">
                 Valid ten minutes, one use. Open
                 <a href="/account/login">the customer sign-in</a> in a private
                 window, enter that number, then this code.
               </p>
             </div>`
          : ''
      }
      ...
      <div class="grid-2" style="align-items:start;">
        ...
        <div class="card card-xl" style="padding:28px;">
          ${sectionHeading('As a customer', 'Send a text')}
          <form method="post" action="/ops/test/text">
            <div class="stack">
              <div class="field">
                <label class="field-label" for="t_phone">From which number</label>
                <input class="input input-lg" type="tel" id="t_phone" name="phone" required
                       value="${escapeHtml(phone)}" list="known-customers">
                <datalist id="known-customers">
                  ${(customers || [])
                    .map(
                      (c) =>
                        `<option value="${escapeHtml(c.phone)}">${escapeHtml(c.name || '')}</option>`
                    )
                    .join('')}
                </datalist>
                <span class="field-hint">A number nobody has signed up with tests what a stranger sees.</span>
              </div>
              <div class="field">
                <label class="field-label" for="t_text">Message</label>
                <input class="input input-lg" type="text" id="t_text" name="text" required
                       placeholder="laundry tomorrow" autofocus>
              </div>
            </div>
            <button class="btn btn-ink btn-lg btn-full" style="margin-top:20px;">
              Send it ${icon('arrow-right', '22')}
            </button>
          </form>

          <p style="font-size:14px;color:var(--ink-500);margin:18px 0 0;">
            Try: <code>laundry tomorrow</code>, <code>cancel</code>,
            <code>how much do i owe</code>, <code>STOP</code>, <code>HELP</code>.
            The full list is in TESTING.md.
          </p>
        </div>

        <div class="card card-xl" style="padding:28px;">
          ${sectionHeading('As a customer', 'Get into the website')}
          <p style="font-size:16px;line-height:1.55;color:var(--ink-700);margin:0 0 20px;">
            Mints a real sign-in code for that customer and shows it here, so
            you can use the booking pages before texting works.
          </p>
          <form method="post" action="/ops/test/code">
            <div class="field">
              <label class="field-label" for="c_phone">Customer's number</label>
              <input class="input input-lg" type="tel" id="c_phone" name="phone" required
                     value="${escapeHtml(phone)}" list="known-customers">
            </div>
            <button class="btn btn-outline btn-lg btn-full" style="margin-top:20px;">
              Give me a code
            </button>
          </form>
          <p style="font-size:14px;color:var(--ink-500);margin:18px 0 0;">
            Every use of this is written to the server log with your name on it.
          </p>
        </div>
      ...
      <div style="margin-top:48px;">
        ${sectionHeading('What was said', `Thread with ${formatPhone(phone)}`)}
        ${
          (thread || []).length
            ? (thread || [])
                .slice()
                ...
                .map(
                  (m) => `
          <div style="padding:12px 0;border-bottom:1px solid var(--ink-100);">
            <div class="eyebrow" style="margin:0 0 4px;">${m.direction === 'INBOUND' ? 'Them' : 'LYNDRY'} &middot; ${dateTime(m.created_at)}</div>
            <div style="font-size:16px;line-height:1.45;">${escapeHtml(m.body)}</div>
          </div>`
                )
                .join('')
            : '<p style="font-size:16px;color:var(--ink-500);margin:0;">Nothing yet.</p>'
        }
      </div>`
          : ''
      }`;

    res.type('html').send(adminPage({ title: 'Test console', active: '/ops/test', body, user: req.opsUser }));
  } catch (err) {
    next(err);
  }
});

router.post('/ops/test/text', guard, may('team.manage'), async (req, res, next) => {
  try {
    const phone = normalisePhone((req.body || {}).phone);
    const text = String((req.body || {}).text || '').trim();

    if (!phone || !text) {
      return res.redirect(303, '/ops/test?error=' + encodeURIComponent('Need a number and a message.'));
    }

    console.log(`TEST TEXT by ${req.opsUser.name}: ${phone} -> "${text}"`);

    // The same handler a real webhook runs. A unique id each time so the
    // duplicate check treats it as a new message.
    await sms.handleInbound({
      providerMessageId: `test-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      from: phone,
      to: config.telnyx.phoneNumber || '+10000000000',
      text,
    });

    // Whatever LYNDRY said back is the newest outbound row for that number.
    ...
    const reply = last && last[0] && last[0].direction === 'OUTBOUND' ? last[0].body : '(no reply — check the server log)';

    return res.redirect(
      303,
      `/ops/test?phone=${encodeURIComponent(phone)}&reply=${encodeURIComponent(reply)}`
    );
  } catch (err) {
    return next(err);
  }
...
router.post('/ops/test/code', guard, may('team.manage'), async (req, res, next) => {
  try {
    const phone = normalisePhone((req.body || {}).phone);
    if (!phone) {
      return res.redirect(303, '/ops/test?error=' + encodeURIComponent('That is not a usable number.'));
    }

    // Impersonation, effectively — so it is named, logged, and admin-only.
    console.warn(`TEST SIGN-IN CODE issued by ${req.opsUser.name} for customer ${phone}`);

    const code = await customerAuth.mintCodeForTesting(phone, req);

    if (!code) {
      return res.redirect(
        303,
        `/ops/test?phone=${encodeURIComponent(phone)}&error=` +
          encodeURIComponent('No active customer with that number.')
      );
    }

    return res.redirect(303, `/ops/test?phone=${encodeURIComponent(phone)}&code=${encodeURIComponent(code)}`);
  } catch (err) {
    return next(err);
  }
});
