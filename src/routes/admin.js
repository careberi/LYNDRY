...
// they were only ever used by those two here. Both are still live and still
// imported by the run, the load-out and the order console, which are the
// screens that actually scan a bag.
const { scannerScript } = require('../web/scanner');
  ...
  try {
    ...
    const _legacyBody = `
      ...
      <!-- THE ACTION CARDS ARE GONE FROM THIS PAGE, at Neil's request.
           ...
           BOTH FUNCTIONS ARE NOW DELETED, and that reverses what this comment
           said. They were kept because Neil said "maybe we'll come back to it
           later" and they were the whole implementation of the step sequence.
           An outside audit read them as live code on 15 September and listed
           them as a workflow this page still owns - which is the cost of
           keeping 570 lines nobody calls: the next reader, human or otherwise,
           cannot tell dead from dormant, and patches the wrong one.

           Neil's call, 16 September: delete the dead ops code. They are in the
           history if they are ever wanted - git log -S pickupSequence - which
           is a better place for a maybe than the file everybody reads. -->
